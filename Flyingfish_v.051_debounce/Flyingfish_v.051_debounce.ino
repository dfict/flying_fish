...
volatile unsigned long lastInterruptTime = 0; // Time of the last interrupt

void changeMode() {
  unsigned long currentTime = millis();
  
  // Check if 50ms have passed since the last interrupt
  if (currentTime - lastInterruptTime > 50) {
    mode = (mode % 4) + 1; // Cycle through modes 1, 2, 3, 4
    modeChanged = true;
    EEPROM.write(0, mode); // Save the mode to EEPROM
...
void setup() {
  ...
  // Set up the external interrupt on BUTTON_PIN
...
void loop() {
  ...
  switch (mode) {
    case 1:
      FlyingFish(weights1, noteTable, 0, 30, 1);
      break;
    case 2:
