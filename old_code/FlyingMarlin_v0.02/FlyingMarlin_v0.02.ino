...
void FlyingMarlin(float* noteTable, int phraseBegin, int phraseEnd, float divisor, float mult, float add) {
  ...
  for (int i = phraseBegin; i > phraseStop; i = i + j) {
    ...
    float mappedTemp = map(tempSensor, 83, 185, 25, 550);
    mappedTemp = mappedTemp / 100;
    
    solarPanel = constrain(solarPanel, 630, 950);
    float mappedSolar = map(solarPanel, 630, 950, 100, 600);
    mappedSolar = mappedSolar / 100; //might not be necessary
    
    LED_PORT ^= 1 << LED_BIT;
    trig = true;
  
    // Use i * mult + add for the Perlin noise calculation
    float noiseValue = (perlinNoise((i * mult) + add) + 1.0) / 2.0;
          ...
          // temp sensor could be tempmult = mappedSolar * mult...?
          // temp sensor could be tempadd = mappedSolar + add...easy
          //test with potentiometer already!
    ...
    int vari = i * 3;
    bond = bond + vari;
    bond = bond / 4; // could be interesting to slow down the sequence based on solar input here.
    delay(bond);
  ...
  switch (mode) {
    case 1:
     FlyingMarlin(noteTable, 0, 30, 1, 5.1, 1.1);
      break;
    case 2:
      FlyingMarlin(noteTable, 30, 60, 4, 2.0, 1.9);
      break;
    case 3:
